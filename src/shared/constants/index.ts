...
export const HOME_BANNER_EMAIL_LINK = 'mailto:schastie.kazan@gmail.com';
...
export type HomeAdvantageTheme = 'light' | 'dark';

export interface HomeAdvantageItem {
  id: string;
  title: string; // без номера
  text: string;
  theme: HomeAdvantageTheme;
  bgImage?: string;
}

export const HOME_ADVANTAGES_TITLE = 'Наши преимущества';
export const HOME_ADVANTAGES_SUBTITLE = 'Почему выбирают именно нас';

export const HOME_ADVANTAGES_ITEMS: HomeAdvantageItem[] = [
  {
    id: 'innovations',
    title: 'Инновации',
    theme: 'dark',
    bgImage: heroImage1,
    text: `Для комфортной организации свадьбы мы внедрили на нашем сайте личные кабинеты для каждой пары. Уже на следующий день после заключения договора мы высылаем логин и пароль. Именно там ведётся вся работа по проекту.

Для нас нет ничего невозможного. Мы ставим задачу и находим решение. Мы за внедрение новых технологий в организацию свадеб. Также мы являемся новаторами в области визуализации свадебных концепций. Об уникальном «Planner Book» читайте в соответствующем блоке.`,
  },
  {
    id: 'guarantees',
    title: 'Гарантии',
    theme: 'light',
    text: `Обратившись в наше свадебное агентство, вы будете уверены, что ваша свадьба пройдёт на высоком уровне, так как мы даём гарантии на свою работу.

Гарантия опыта, гарантия успеха и гарантия возврата денег зафиксированы в нашем договоре, который мы заключаем с каждым клиентом.

Ваша свадьба в надёжных руках. Репутация для нас — превыше всего. Подробные условия гарантий читайте в разделе «Услуги и цены».`,
  },
  {
    id: 'calm',
    title: 'Спокойствие',
    theme: 'light',
    text: `Вы не должны переживать, бояться и тратить свои силы. Мы сделаем так, чтобы вы наслаждались каждым моментом подготовки. Вы сами определяете процент включённости в процесс организации вашей свадьбы. Будете ли вы выбирать с нами ткань и цветы для оформления, бумагу для полиграфии, дегустировать начинки для свадебного торта или предпочтёте утвердить главные позиции дистанционно — решать вам.

Мы с радостью учтём все ваши пожелания и составим индивидуальный план работы. В процессе работы мы держим постоянную связь через удобное для вас средство коммуникации: e-mail, WhatsApp, Skype, Вконтакте или Facebook.`,
  },
  {
    id: 'experience',
    title: 'Опыт',
    theme: 'dark',
    bgImage: heroImage2,
    text: `Наше свадебное агентство одно из первых, кто стал специализироваться исключительно на свадьбах с выездными церемониями. Мы не по картинкам из Pinterest и Instagram знаем, как пришла и ушла мода на стулья в чехлах и ширмы в качестве бэка президиума, на оформление помпонами и пенопластовыми буквами, на маленькие арки со свисающей тканью, на фотозоны в стиле чаепития с фотобутафорией и на многое другое. Мы всё это проходили сами. И поэтому с годами поняли, как делать свадьбы вне времени.

Опыт — это ещё и накопленные знания о выходах из тех или иных форс-мажорных ситуаций. Погодные обстоятельства, технические сбои, личностные факторы — нет такой свадьбы, с которой мы бы не справились. Наша команда готова ко всему! В день свадьбы мы слышим только положительные отзывы от пар, потому что знаем, как предвосхитить их ожидания.`,
  },
  {
    id: 'scenario',
    title: 'Сценарий',
    theme: 'light',
    text: `Для каждой свадьбы мы пишем уникальный сценарий. Для этого Елена и Людмила возьмут подробное интервью о вас и ваших гостях. Такого лёгкого, непринуждённого, но столь полезного интервью у вас ещё точно не было в жизни.

Два часа за чашечкой чая или кофе пролетают незаметно. Истории из детства, юности, знакомство со второй половинкой, приключения с друзьями помогают спрогнозировать эмоции для создания правильной атмосферы. Благодаря авторскому сценарию свадьба станет настоящим семейным событием, а не просто вечером с конкурсами и концертом из артистов.`,
  },
  {
    id: 'planner-book',
    title: 'Planner Book',
    theme: 'light',
    text: `Для каждой свадьбы мы создаём уникальный «Planner Book». Это подробная визуализация проекта, которая отвечает на вопрос: почему тот или иной человек, объект, деталь должны быть в концепции этой свадьбы.

Мы считаем, что двух одинаковых свадеб просто не существует, потому что концепция выстраивается исходя из истории будущих молодожёнов и индивидуальности пары. «Planner Book» — это большая книга, которая остаётся на память у пары о времени подготовки первого семейного праздника.`,
  },
  {
    id: 'budgets',
    title: 'Бюджеты',
    theme: 'dark',
    bgImage: heroImage3,
    text: `Мы занимаемся организацией свадеб с бюджетами от 3 млн рублей за фиксированный гонорар. Наше свадебное агентство знает, как сделать свадьбу на 3 миллиона так, чтобы она выглядела как минимум на 4. Черновой вариант сметы мы рассчитываем уже на первой встрече. Для этого нам необходимо услышать ваши пожелания и предполагаемый бюджет свадьбы, чтобы правильно расставить приоритеты.

Итоговая сумма будет зависеть от выбранных позиций. За годы работы мы научились вписываться в озвученный бюджет и оптимизировать сметы не в ущерб картинке и атмосфере свадебного дня.`,
  },
  {
    id: 'saving',
    title: 'Экономия',
    theme: 'light',
    text: `Мы экономим ваше время, здоровье и деньги. Свадебное агентство Ксении Мироновой знает, как сделать современную стильную свадьбу в Москве и Подмосковье недорого. Правильно распределяя ваш запланированный бюджет, мы учтём все пожелания и поделимся советами, на чём можно сэкономить, а на чём всё-таки не стоит.

Мы не накручиваем стоимость свадебных услуг. Наши сметы максимально прозрачные, а презентации по специалистам всегда содержат ссылки на их личные сайты.`,
  },
  {
    id: 'online-offline',
    title: 'Online / Offline',
    theme: 'light',
    text: `Организация свадьбы под ключ — это сложный процесс, требующий постоянной коммуникации с парой и подрядчиками. Для выбора площадки необходимо выехать на предполагаемое место для согласования технических моментов, зонирования и утверждения меню.

Выбор свадебных подрядчиков состоит из нескольких этапов: онлайн-знакомство с предлагаемыми кандидатурами, встреча-знакомство и подписание договора. В случае привлечения региональных специалистов или невозможности личной встречи согласование происходит дистанционно. В процессе планирования свадьбы мы также нередко используем сервисы Skype и Zoom для онлайн-конференций.`,
  },
  {
    id: 'team',
    title: 'Команда',
    theme: 'dark',
    bgImage: heroImage4,
    text: `Каждой паре мы предлагаем от трёх специалистов в каждой категории, подходящих по ценовому сегменту и стилистическим особенностям. С нами вы экономите минимум до 430 часов — именно столько вам потребуется, чтобы самостоятельно найти, встретиться и определиться, подойдёт ли вам этот специалист или нет.

Мы уже нашли тех, благодаря кому ваша свадьба станет идеальной. Наше свадебное агентство даёт гарантии на услуги специалистов из разных сфер, которых мы рекомендуем. Мы понимаем, что несём большую ответственность при организации свадьбы «под ключ».`,
  },
  {
    id: 'timing',
    title: 'Сроки',
    theme: 'light',
    text: `Планировать свадьбу лучше заранее. У нас есть опыт организации свадьбы как за полгода, так и за месяц, но для предоставления высокого сервиса лучше обратиться в свадебное агентство заранее.

Придя к нам в летний период, вы рискуете остаться без подходящей площадки, так как бронирование дат начинается в осенне-зимний сезон. При подготовке свадьбы мы придерживаемся указанных в договоре дедлайнов. Сроки утверждения специалистов могут сдвинуться лишь в случае невозможности встретиться в указанные даты.`,
  },
  {
    id: 'black-list',
    title: 'Black List',
    theme: 'light',
    text: `Раскроем вам тайны. Свадебные организаторы Москвы и Подмосковья несколько лет назад объединились в чаты. Там мы обмениваемся с коллегами историями о недобросовестных подрядчиках, нарушающих условия работы, и узнаём о новых ограничениях на тех или иных площадках.

Так как свадебная индустрия очень большая (одних только кавер-групп зарегистрировано порядка 20 тысяч), этот чат позволяет оперативно собирать отзывы и актуализировать информацию в наших базах. За 10 лет работы сотни специалистов из разных сфер попали в чёрный список из-за несоблюдения договора или халатного отношения к работе.`,
  },
  {
    id: 'wedding-school',
    title: 'Wedding School',
    theme: 'dark',
    bgImage: heroImage1,
    text: `Мы являемся создателями одной из крупнейших школ по подготовке свадебных организаторов и координаторов — «Moscow Wedding School», открывшейся в 2013 году.

За годы работы мы выпустили более 400 начинающих специалистов из России, Беларуси, Казахстана, Латвии и даже Бельгии. География российских городов огромна: от Москвы и Санкт-Петербурга до Владивостока, Сочи и Севастополя.

Интенсивные курсы и мастер-классы проходят как в онлайн-, так и в офлайн-форматах. В стоимость обучения входят диплом, пакет документации и стажировка на реальных проектах.`,
  },
  {
    id: 'experts',
    title: 'Эксперты',
    theme: 'light',
    text: `Мы хотим, чтобы свадебная индустрия в нашей стране развивалась, и поэтому делимся знаниями и опытом с начинающими специалистами. Руководителя агентства Ксению Миронову приглашают в качестве спикера на самые крупные мероприятия для свадебных профессионалов. Также Ксения является автором и ведущей первого свадебного онлайн-телеканала «Russian Wedding TV».

В эфирах можно найти полезные советы для будущих молодожёнов и сюжеты о специалистах. Для объединения коллег мы создаём мероприятия в рамках «Russian Wedding Club». С 2015 года мы организовали более 10 встреч, которые посетили около тысячи человек, работающих в свадебной индустрии.`,
  },
  {
    id: 'all-year',
    title: 'Круглый год',
    theme: 'light',
    text: `Мы не те свадебные организаторы, кто работает только летом в качестве хобби. Мы занимаемся организацией летних, осенних, зимних и весенних свадеб — каждое время года имеет свои преимущества.

Зная нюансы работы площадок, особенности сезонности цветов, а также выгодные условия в межсезонье, мы предложим лучшие варианты именно для вашей свадьбы.`,
  },
  {
    id: 'tv',
    title: 'Эфиры на ТВ',
    theme: 'dark',
    bgImage: heroImage2,
    text: `Мы выступаем в качестве свадебных экспертов на телеканалах: «Россия», НТВ, «Пятница», «360 Подмосковье», «Москва 24».

Наши свадьбы публикуют в свадебных журналах, блогах и тематических пабликах.`,
  },
];
...
// ... здесь уже есть импорты и константы
...
export const HOME_NUMBERS_BACKGROUND = heroImage1;
